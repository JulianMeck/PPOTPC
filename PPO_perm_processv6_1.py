...
def fit_sines2(up, dwn, t_raw, Tmax, Tmin, return_errors=False):
    ...
    guess_amp_up = (np.max(up) - np.min(up)) / 2
    guess_amp_dwn = (np.max(dwn) - np.min(dwn)) / 2
    guess_offset_up = np.mean(up)
    guess_offset_dwn = np.mean(dwn)
    ...
    initial_guesses = [guess_freq, guess_amp_up, 0.0, guess_offset_up, guess_amp_dwn, 0.0, guess_offset_dwn, 0.0]
